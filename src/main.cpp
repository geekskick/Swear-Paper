...
int main(int argc, const char *argv[]) {
    ...
    (void)argv;
    reddit s{"earthporn"};
    simple_url swears{"https://raw.githubusercontent.com/LDNOOBW/List-of-Dirty-Naughty-Obscene-and-Otherwise-Bad-Words/master/en"};

    std::shared_ptr<program_delegate_b> vb{ std::make_shared<verbose_program_delegate>()};
    std::shared_ptr<downloader_delegate_b> dd { std::make_shared<downloader_delegate>(vb)};

    std::cout << s.url() << std::endl;
    std::cout << swears.url() << std::endl;

    std::cout << curl_downloader(dd).get_vec(swears.url()).size() << std::endl;
#ifdef DEBUG
    std::cout << curl_downloader(dd).get_str(s.url())<< std::endl;
#endif
    earthporn_parse bp{};
    const auto img_url {subreddit(curl_downloader(dd).get_str(s.url()), bp).url()};
    std::cout << img_url << std::endl;

#ifdef DEBUG
    const auto img {curl_downloader(dd).get_img(img_url)};
    ...
    // auto listl = d.perform_vector(swear_url, swearwords);
