...
int main(int argc, const char *argv[]) {
    std::shared_ptr<downloader_delegate_b> download_del{std::make_unique<downloader_delegate>()};
    std::shared_ptr<json_parse_delegate_b> parse_del{std::make_unique<json_parse_delegate>()};
    ...
    desc.add_options()("help", "Display help message")("swears", po::value<std::string>(), "Specify the location of the swear word list")(
    ...
    if (!list.first) {
        std::cerr << ansi_codes::red << "----\t" << list.second << ansi_codes::reset << std::endl;
        exit(EXIT_SUCCESS);
    }

    // If the swear words list isn't populated set the appropriate flag
    if (swearwords.empty()) {
        std::cerr << ansi_codes::red << "----\tSwear words not populated" << ansi_codes::reset << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    try {
        ...
        if (!rc.first) {
            std::cerr << ansi_codes::red << "----\t" << rc.second << ansi_codes::reset << std::endl;
            exit(EXIT_FAILURE);
        }
    ...
    do {
        if (get_image(d, e, raw_image, json_str, idx)) {
            ...
            if (swear_copy.size() == 0) {
                std::cout << "----\tNo more swear words left to try and fit on, trying a different image." << std::endl;
                raw_image.clear();
                retry = true;
            }
        }
    ...
    std::cout << ansi_codes::green << "----\tDone, you " << word << ansi_codes::reset << std::endl;
}
