...
void image::put_text(const std::string &word) {
    ...
    cv::putText(m_image, word, cv::Point(word_loc.x, word_loc.y), CV_FONT_HERSHEY_SCRIPT_COMPLEX, scale_factor(), text_colour(word),
                m_line_thickness);
}

void image::save_to_file(const std::string &filename) { cv::imwrite(filename, m_image); }
...
int image::scale_factor(void) {
    ...
    int text_width{cv::getTextSize("f", CV_FONT_HERSHEY_SCRIPT_COMPLEX, 1, m_line_thickness, nullptr).width};
