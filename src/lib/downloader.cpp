...
size_t downloader::write_data_to_string(void *ptr, size_t size, size_t nmemb,
  ...
  *static_cast<std::string *>(stream) +=
      data;  // add the data to the out string
  return size * nmemb;
}

// Do the download and get the reply as a string
std::optional<std::string> downloader::perform_string(
  ...
  check_rc(
      code,
      "url_easy_setopt(m_curl, CURLOPT_WRITEFUNCTION, write_data_to_string)");
...
void downloader::check_rc(const CURLcode &rc, const std::string &msg) const {
  if (CURLE_OK != rc) {
    throw std::runtime_error(std::string(msg + std::string(": ") +
                                         std::string(curl_easy_strerror(rc))));
  }
}

// Do the download and get the reply as a vector of strings
std::optional<std::vector<std::string>> downloader::perform_vector(
    const std::string &url) const {
...
std::optional<std::vector<char>> downloader::perform_image(
    const std::string &url) const {
  auto code = CURLcode{};
